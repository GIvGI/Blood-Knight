...
function Enemy:new(x,y)
    ...
    obj.wanderLength = 200 --Maximum distance he can randomly go to in a single step.
    obj.PlayerStates = {RUN = "run", DEAD = "dead", ATTACK = "attack"}
...
-- #region auxiliary variables    
    ...
    obj.wanderPointX = obj.x + 5
    obj.wanderPointY = obj.y + 5
...
-- #region Animations
        ...
        animationTimer = 0,
        frameRate = 0.2,
        spriteSheet = love.graphics.newImage("Sprites/Knight3/Walk.png")
    }
    for i = 0, 7 do
        ...
        animationTimer = 0,
        frameRate = 0.2,
        spriteSheet = love.graphics.newImage("Sprites/Knight3/Attack.png")
    }
    for i = 0, 3 do
        ...
        animationTimer = 0,
        frameRate = 0.2,
        spriteSheet = love.graphics.newImage("Sprites/Knight3/Dead.png")
    }
    for i = 0, 5 do
...
function Enemy:Wander(dt)
    local dx = self.wanderPointX - self.x 
    local dy = self.wanderPointY - self.y
    local distance = math.sqrt(dx^2 + dy^2)
    local angle = math.atan2(dy,dx)
    if distance > 1 then
        if dx < 0 then self.direction = -1 end
        ...
        self.wanderPointX = math.random(self.x - self.wanderLength, self.x + self.wanderLength)
        self.wanderPointY = math.random(self.y - self.wanderLength, self.y + self.wanderLength)
    end
end
